...
/**quizzes */
@Injectable()
export class AuthQuizPacketsGuard implements CanActivate, CanActivateChild {
    redirectUrl: string;
    user: Meteor.User;
    constructor(private router: Router,
        private authService: AuthService) { }
    ...
    checkLogin(url: string): boolean {       
        if (!!(this.authService.user && Roles.userIsInRole(this.authService.user, ['editor', 'member']))) {
            /**normal */
            return true;
        } else {
            this.authService.testLogin().subscribe(() => {
